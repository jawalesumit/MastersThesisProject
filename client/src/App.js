        ...
        datasets: [
          {
            ...
            hoverBackgroundColor: [
              SENTIMENT_POSITIVE_COLOR_LIGHT,
              SENTIMENT_NEGATIVE_COLOR_LIGHT
            ]
          }
        ]
      },
    ...
    return (
      <div className="App">
        <Container>
              ...
              <Form.Row>
                <Col sm={{ span: 12 }}>
                  <Button
                    style={{ margin: "0.2em" }}
                    variant="primary"
                    disabled={this.state.analyzeDisabled}
                    onClick={
                      !this.state.isLoading ? e => this.analyseQueries(e) : null
                    }
                  >
          ...
          <Row className="show-grid justify-content-center">
            <div id="alert_msgs" style={{ margin: "1em" }}>
              <Alert
                id="invalid_alert"
                dismissible
                show={this.state.showInvalidInputAlert}
                variant="danger"
                onClose={e => this.setState({ showInvalidInputAlert: false })}
              >
                <Alert.Heading>Oh snap! You got an error!</Alert.Heading>
                <p>The input cannot be blank!</p>
              </Alert>
              <Alert
                id="fetch_alert"
                show={this.state.showFetchAlert}
                variant="secondary"
                onClose={e => this.setState({ showFetchAlert: false })}
              >
                <p>Finding related tweets...</p>
              </Alert>
              <Alert
                id="success_alert"
                show={this.state.showSuccessInputAlert}
                variant="primary"
                onClose={e => this.setState({ showSuccessInputAlert: false })}
              >
                <Alert.Heading>
                  Found{" "}
                  {this.state.mnb_sentiment_positive +
                    this.state.mnb_sentiment_negative}{" "}
                  related tweets!
                </Alert.Heading>
              </Alert>
            </div>
          ...
          <Modal
            size="lg"
            show={this.state.showTweets}
            onHide={e => this.setState({ showTweets: false })}
          >
            ...
            <Modal.Body>
              <Table responsive striped bordered>
                ...
                <tbody>{this.state.modalTableBody}</tbody>
              </Table>
            </Modal.Body>
            <Modal.Footer>
              <Button
                variant="secondary"
                onClick={e => this.setState({ showTweets: false })}
              >
                Close
              </Button>
            </Modal.Footer>
  ...
  showTweets(event, modelName) {
    ...
    if (event.length !== 0) {
      ...
      if (modelName === MNB)
        ...
        else modalBody = this.state.mnb_sentiment_negative_tweets_table;
      else if (modelName === SVM)
        if (tempSentiment === SENTIMENT_POSITIVE)
          modalBody = this.state.svm_sentiment_positive_tweets_table;
        else modalBody = this.state.svm_sentiment_negative_tweets_table;
      else if (modelName === LR)
        if (tempSentiment === SENTIMENT_POSITIVE)
          modalBody = this.state.lr_sentiment_positive_tweets_table;
        else modalBody = this.state.lr_sentiment_negative_tweets_table;
      ...
      .then(res => {
        ...
        if (result.length === 0) {
          ...
          toDisplay_mnb = (
            <Doughnut
              width={250}
              data={this.state.sentimentChart_MNB}
              legend={{ display: true }}
              redraw
              onElementsClick={e => this.showTweets(e, MNB)}
            />
          );
          ...
          toDisplay_svm = (
            <Doughnut
              width={250}
              data={this.state.sentimentChart_SVM}
              legend={{ display: true }}
              redraw
              onElementsClick={e => this.showTweets(e, SVM)}
            />
          );
          ...
          toDisplay_lr = (
            <Doughnut
              width={250}
              data={this.state.sentimentChart_LR}
              legend={{ display: true }}
              redraw
              onElementsClick={e => this.showTweets(e, LR)}
            />
          );
          ...
          this.setState({
            mnb_sentiment_positive_tweets_table: mnb_pos_tweets
          });
          this.setState({
            mnb_sentiment_negative_tweets_table: mnb_neg_tweets
          });
          ...
          this.setState({
            svm_sentiment_positive_tweets_table: svm_pos_tweets
          });
          this.setState({
            svm_sentiment_negative_tweets_table: svm_neg_tweets
          });
        ...
        ReactDOM.render(
          toDisplay_mnb,
          document.getElementById("SentimentChart_MNB")
        );

        ReactDOM.render(
          toDisplay_svm,
          document.getElementById("SentimentChart_SVM")
        );

        ReactDOM.render(
          toDisplay_lr,
          document.getElementById("SentimentChart_LR")
        );
